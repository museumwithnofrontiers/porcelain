...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('porcelain.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('porcelain.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('porcelain.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
