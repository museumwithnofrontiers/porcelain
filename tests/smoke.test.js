...
describe('website smoke test', () => {
  ...
  it('colours and names the source-database chip from the manifest projects section', async () => {
    const { app, host } = await mountSite('#/item/f88a365b-0b19-5fa2-8be0-31a30d035b7c')
  ...
  it('shows the explore-partner notice only for the project dataset.config.js lists', async () => {
    const epm = await mountSite('#/item/3d81aec6-b59f-5b48-b482-d6cde0cf5bda')
    ...
    const isl = await mountSite('#/item/f88a365b-0b19-5fa2-8be0-31a30d035b7c')
  ...
  it('renders the related-database and artistic-introduction links from the manifest', async () => {
    const [items] = await loadEntities(['items'])
    const item = items.find((i) => i.id === 'f88a365b-0b19-5fa2-8be0-31a30d035b7c')
  ...
  it('shows the source project on a collection-results tile, from the manifest', async () => {
    const [items] = await loadEntities(['items'])
    const islProjectId = "76eaf6c2-8025-53bc-9e39-106803a3917e"
    const item = items.find((i) => i.project_id === islProjectId) ?? { internal_name: "Breakfast Set" }
    const { app, host } = await mountSite(`#/search?q=${encodeURIComponent(item.internal_name.replace(/[*_]/g, ''))}`)
    await vi.waitFor(() => expect(host.querySelector('.mwnf-grid__tile')).not.toBeNull(), { timeout: 20000 })
    expect(host.textContent).toContain("for project Discover Baroque Art")
    app.unmount()
  }, 30000)
  ...
  it('renders the layout glossary tool and dynasty popouts on the item sheet', async () => {
    ...
    const { app, host } = await mountSite('#/item/cfa923e7-7677-5dde-b481-034397401b10')
  ...
  it('renders the partners list on the composed partner-list view', async () => {
    ...
    expect(host.textContent).toContain("Kuwait")
    expect(host.textContent).toContain('Dar al-Athar al-Islamiyyah')
  ...
  it('renders a partner profile on the composed record view', async () => {
    const { app, host } = await mountSite('#/partner/95483cda-d367-5af3-ad0e-2779b97409cb')
    ...
    await vi.waitFor(() => expect(host.textContent).toContain('Dar al-Athar al-Islamiyyah'), { timeout: 20000 })
    await vi.waitFor(() => expect(host.textContent).toContain("Kuwait City"), { timeout: 20000 })
  ...
  it("renders a partner's objects on the composed grid results view", async () => {
    const { app, host } = await mountSite('#/partner/95483cda-d367-5af3-ad0e-2779b97409cb/objects')
  ...
  it('renders the shared texts and its own over them', async () => {
    ...
    expect(checkTextsRendered(host, {
      namespaces: ['porcelain', 'core', 'layout', 'catalogue', 'record', 'sheet', 'timeline', 'partner', 'gallery'],
    })).toEqual([])
